...
Furthermore, following the original paper, all layer weights are initialized with Xavier initialization \cite{glorot2010understanding}, while biases are initialized with $0$. The datasets are split into training, testing and validation sets with 80/10/10\% splits. The models are trained for 1000 epochs, with a learning rate of $1.0 \times 10^{-3}$ and Adam \cite{kingma2014adam} is used as an optimizer.
...
We want to stress that though not documented in the paper, early stopping is utilized in the downstream model training. This is very important for a fair comparison of the models, as the explanations of an overfit downstream model vary from the "best" one. TODO: Details

TODO: DISCUSS DIFFERENCE OF GRAPH LAYERS HERE AND MOTIVATE WHY WE USE HIGHER-ORDER GNN!
GraphConv to allow passing of edge weights(+ bipartite)!?

\textbf{Downstream model specifics}:

- Therefore, assuming default settings for layers -> embedding normalization! (TODO), default order 'AW'(see 4.17)
